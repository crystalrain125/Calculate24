				...
				for(LinkedList<Integer> permutation: permutations) {
					for(String op1: operators) {
						for(String op2:operators) {
							for(String op3: operators) { //ugly way of hard-coding the answers; will work on the recursive method make the calculator work for more than sets of 4 numbers
								...
								if(calculate(arr1) == 24) {
									solutions.add(arr1); 
								} else if (calculate(arr2) == 24) {
									solutions.add(arr2);
								}
							}
						}
					}
					//find addition/sub/mult/div combos
					//check, if == 24, add to solutions
